...
5. 첫 번역 때 기본 M2M100 모델 파일을 WASM에서는 약 650MB, WebGPU에서는 약 750MB 내려받습니다. WebGPU 실패 후 자동 폴백하면 두 경로를 순차 다운로드해 최초 전송량이 최대 약 1.4GB가 될 수 있으며, 실패한 WebGPU 가중치는 확장 Cache Storage에서 정리합니다. 첫 듣기 때는 Supertonic 3 모델과 기본 음성 스타일 약 400MB를 별도로 내려받습니다. 성공한 경로의 모델은 이후 Chrome HTTP 캐시에서 재사용됩니다.
